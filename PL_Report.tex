...
リスト内に特定の要素が存在するかどうかを判定する \verb|listexi| 関数を実装した．
\verb|C|言語における文字列検索関数 \verb|strchr| に相当する機能を，整数リストを対象として実現している．

この関数は，整数とリストを引数に取り，リスト内にその整数が存在すれば \verb|true|，存在しなければ \verb|false| を返す．
パターンマッチングを使用し，空リストに到達した場合には \verb|false| を返す終了条件とし，先頭要素が一致すれば \verb|true| を返す．
一致しない場合には再帰的に残りのリストに対して同じ判定を行う．
